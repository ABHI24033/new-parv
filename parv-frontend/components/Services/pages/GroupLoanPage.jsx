...
import { GroupLoanData } from "../data/GroupLoanData";

function AboutGroupLoan({ item }) {
    return (
        <header className="mb-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">{item?.question}</h2>
        ...
        <div>
            <div className="group relative w-full bg-gray-100 rounded-2xl p-4 transition-all duration-500 max-md:max-w-md max-md:mx-auto md:h-64 xl:p-7 hover:bg-indigo-600">
                <div className="bg-white rounded-full flex justify-center items-center mb-5 w-14 h-14">
                    <img src={data?.icon} alt="icons" className="rounded-full" />
                </div>
...
function Features({ data }) {
    return (
        <section className="mb-12">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">Features of Group Loan</h2>
...
function LoanTypes({ items }) {
    return (
        <section className="mb-12">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">Types of Group Loans</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {items?.map((item, index) => (
                        ...
                        <ul className="ml-6 space-y-1">
                            {item?.details.map((detail, idx) => (
                                <li key={idx} className="flex gap-2 items-center">
                                    ...
                                    <span className="w-[90%]">{detail}</span>
                                </li>
                            ))}
                        </ul>
                    </li>
...
export default function GroupLoanPage() {
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto w-full bg-gray-50">
            <div>
                <AboutGroupLoan item={GroupLoanData} />
                <Features data={GroupLoanData?.features} />
                <Eligibility items={GroupLoanData?.eligibility} />
                <LoanTypes items={GroupLoanData?.loanTypes} />
                <AdditionalDetails items={GroupLoanData?.additionalBenefits} />
                <DocumentsRequired items={GroupLoanData?.documents} />
            </div>
        </div>
    );
}
